    ...
    Screen(
        bottom=bar.Bar(
            [
                ...
                widget.Spacer(bar.STRETCH),
                widget.Sep(),
                #  widget.GenPollText(update_interval=60, func=lambda: check_output('panel -w'.split()).decode("utf-8")),
                #  widget.Battery(update_interval=300, format='🔋 {percent:2.0%}'),
                widget.Volume(fmt='🎧 {}'),
                widget.Sep(),
                widget.GenPollText(update_interval=15, func=lambda: check_output('panel -w'.split()).decode("utf-8")),
                #  widget.Wlan(update_interval=5, disconnected_message='❌ 00/70', format='🌏 {quality}/70'),
                widget.Sep(),
